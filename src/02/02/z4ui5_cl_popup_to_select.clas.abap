CLASS z4ui5_cl_popup_to_select DEFINITION
  PUBLIC
  FINAL
  CREATE PROTECTED .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory
      ...
      RETURNING
        VALUE(r_result)     TYPE REF TO z4ui5_cl_popup_to_select .
    METHODS result
      RETURNING
...
CLASS z4ui5_cl_popup_to_select IMPLEMENTATION.
...
* +--------------------------------------------------------------------------------------</SIGNATURE>
  METHOD factory.
    ...
    r_result->mr_tab = z4ui5_cl_util=>conv_copy_ref_data( i_tab ).
  ...
  METHOD display.
    ...
    DATA(tab) = popup->table_select_dialog(
              items   = `{path:'`
                                ...
                                && z4ui5_cl_util=>boolean_abap_2_json( me->descending )
                                && ` } }`
    ...
    DATA(lt_comp) = z4ui5_cl_util=>rtti_get_t_attri_by_struc( <tab_out> ).
    ...
    LOOP AT lt_comp INTO ls_comp.
      DATA(text) = COND #(
                     LET data_element_name = substring_after( val = CAST cl_abap_elemdescr( ls_comp-type )->absolute_name sub = '\TYPE=' )
                         medium_label = z4ui5_cl_util=>rtti_get_data_element_texts( data_element_name )-medium IN
