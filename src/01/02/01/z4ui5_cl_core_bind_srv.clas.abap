CLASS z4ui5_cl_core_bind_srv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA mo_app    TYPE REF TO z4ui5_cl_core_app .
    DATA mr_attri  TYPE REF TO z4ui5_if_core_types=>ty_s_attri .
    DATA ms_config TYPE z4ui5_if_core_types=>ty_s_bind_config .
    DATA mv_type   TYPE string .

    METHODS constructor
      IMPORTING
        !app TYPE REF TO z4ui5_cl_core_app .

    METHODS main_local
      IMPORTING
        !val          TYPE data
        !config       TYPE z4ui5_if_core_types=>ty_s_bind_config OPTIONAL
      RETURNING
...
CLASS z4ui5_cl_core_bind_srv IMPLEMENTATION.
  ...
  METHOD bind_tab_cell.
    ...
    DATA(lt_attri) = z4ui5_cl_util=>rtti_get_t_attri_by_struc( ms_config-tab ).
  ...
  METHOD get_client_name.
    ...
    result = COND #( WHEN mv_type = z4ui5_if_core_types=>cs_bind_type-two_way
        THEN `/` && z4ui5_if_core_types=>cs_ui5-two_way_model )
        && `/` && result.
  ...
  METHOD main.
    ...
    IF z4ui5_cl_util=>check_bound_a_not_inital( config-tab ).
    ...
    IF `/` && z4ui5_if_core_types=>cs_ui5-two_way_model = result.
      RAISE EXCEPTION TYPE z4ui5_cx_util_error
        EXPORTING
  ...
  METHOD main_cell.
    ...
    DATA(lo_bind) = NEW z4ui5_cl_core_bind_srv( mo_app ).
  ...
  METHOD main_local.
    TRY.
        ...
        DATA(lo_json) = CAST z4ui5_if_ajson( z4ui5_cl_ajson=>new( ) ).
        lo_json->set( iv_path = `/` iv_val = val ).

        IF config-custom_mapper IS BOUND.
          lo_json = lo_json->map( config-custom_mapper ).
        ELSE.
          lo_json = lo_json->map( z4ui5_cl_ajson_mapping=>create_upper_case( ) ).
        ENDIF.

        IF config-custom_filter IS BOUND.
          lo_json = lo_json->filter( config-custom_filter ).
        ELSE.
          lo_json = lo_json->filter( z4ui5_cl_ajson_filter_lib=>create_empty_filter( ) ).
        ENDIF.

        DATA(lv_id) = to_upper( z4ui5_cl_util=>uuid_get_c22( ) ).
        INSERT VALUE #( name_client     = |/{ lv_id }|
                        name            = lv_id
                        json_bind_local = lo_json
                        bind_type       = z4ui5_if_core_types=>cs_bind_type-one_time )
  ...
  METHOD update_model_attri.
    ...
    mr_attri->view          = COND #( WHEN ms_config-view IS INITIAL THEN z4ui5_if_client=>cs_view-main ELSE ms_config-view ).
