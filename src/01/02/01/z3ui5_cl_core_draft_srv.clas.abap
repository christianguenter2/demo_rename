CLASS z3ui5_cl_core_draft_srv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS create
      IMPORTING
        !draft     TYPE z3ui5_if_types=>ty_s_draft
    ...
    METHODS read_draft
      IMPORTING
        !id           TYPE clike
      RETURNING
        VALUE(result) TYPE z3ui5_if_core_types=>ty_s_db.

    METHODS read_info
      IMPORTING
        !id           TYPE clike
      RETURNING
        VALUE(result) TYPE z3ui5_if_types=>ty_s_draft.
...
CLASS z3ui5_cl_core_draft_srv IMPLEMENTATION.


  METHOD cleanup.

    DATA(lv_four_hours_ago) = z3ui5_cl_util=>time_substract_seconds(
        time    = z3ui5_cl_util=>time_get_timestampl( )
        seconds = 60 * 60 * 4 ).

    DELETE FROM z3ui5_t_core_01 WHERE timestampl < @lv_four_hours_ago.
    COMMIT WORK.
  ...
  METHOD create.
    ...
    DATA(ls_db) = VALUE z3ui5_if_core_types=>ty_s_db(
        id                = draft-id
        id_prev           = draft-id_prev
        id_prev_app       = draft-id_prev_app
        id_prev_app_stack = draft-id_prev_app_stack
        uname             = z3ui5_cl_util=>user_get_tech( )
        timestampl        = z3ui5_cl_util=>time_get_timestampl( )
        data              = model_xml ).

    MODIFY z3ui5_t_core_01 FROM @ls_db.
    IF sy-subrc <> 0.
