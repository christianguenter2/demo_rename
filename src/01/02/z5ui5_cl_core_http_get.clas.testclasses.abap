...
CLASS z5ui5_cl_core_http_get DEFINITION LOCAL FRIENDS ltcl_test_http_get.

CLASS ltcl_test_http_get IMPLEMENTATION.

  METHOD file_not_initial.

    DATA(lo_get) = NEW z5ui5_cl_core_http_get( ).
...
*    ENDIF.
  ...
  METHOD debugging_tools.
    ...
    DATA(lv_index_html) = to_upper( z5ui5_cl_fw_cc_debugging_tools=>get_js( ) ) ##NEEDED.
    IF lv_index_html CS `<`.
