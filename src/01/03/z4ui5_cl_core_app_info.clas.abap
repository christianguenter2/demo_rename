CLASS z4ui5_cl_core_app_info DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
*    DATA mv_device_gav TYPE string.
    ...
    CLASS-METHODS factory
      RETURNING
        VALUE(result) TYPE REF TO z4ui5_cl_core_app_info.
...
CLASS z4ui5_cl_core_app_info IMPLEMENTATION.
  ...
  METHOD view_display_start.
    ...
    DATA(page2) = z4ui5_cl_xml_view=>factory_popup(
         )->dialog(
*            stretch    = abap_true
...
*   simple_form2->text( client->_bind( mv_device_type ) ).
    ...
    simple_form2->checkbox( enabled = abap_false selected = z4ui5_cl_util=>rtti_check_lang_version_cloud( ) ).

    DATA(lv_count) = CONV string( NEW z4ui5_cl_core_draft_srv( )->count_entries( ) ).
    simple_form2->toolbar( )->title( `abap2UI5` ).
    simple_form2->label( `Version ` ).
    simple_form2->text( z4ui5_if_app=>version ).
