CLASS z5ui5_cl_ui5 DEFINITION
  ...
  PUBLIC SECTION.
    CLASS-METHODS _factory
      ...
      RETURNING VALUE(result) TYPE REF TO z5ui5_cl_ui5.
    ...
    METHODS _add
      IMPORTING n             TYPE clike
                ns            TYPE clike
                t_p           TYPE z5ui5_if_types=>ty_t_name_value OPTIONAL
      RETURNING VALUE(result) TYPE REF TO z5ui5_cl_ui5.
    ...
    METHODS _ns_z2ui5
      RETURNING VALUE(result) TYPE REF TO z5ui5_cl_ui5_z2ui5.

    METHODS _ns_html
      RETURNING VALUE(result) TYPE REF TO z5ui5_cl_ui5_html.

    METHODS _ns_webc
      RETURNING VALUE(result) TYPE REF TO z5ui5_cl_ui5_webc.

    METHODS constructor
      IMPORTING node TYPE REF TO z5ui5_cl_ui5__tree OPTIONAL.

    METHODS _stringify
      RETURNING VALUE(result) TYPE string.

  PROTECTED SECTION.
    DATA _node TYPE REF TO z5ui5_cl_ui5__tree.

    CLASS-METHODS _2xml
      IMPORTING obj           TYPE REF TO z5ui5_cl_ui5
      RETURNING VALUE(result) TYPE string.
...
CLASS z5ui5_cl_ui5 IMPLEMENTATION.
  METHOD constructor.
  ...
  METHOD _2bool.
    result = z5ui5_cl_util=>boolean_abap_2_json( val ).
  ENDMETHOD.
...
*                       ( n = `xmlns:webc`      v = `sap.ui.webc.main` )
    ...
    LOOP AT obj->_node->mt_child INTO DATA(lr_child).
      DATA(lo_child) = NEW z5ui5_cl_ui5( lr_child ).
  ...
  METHOD _add.
    ...
    DATA(lo_node) = NEW z5ui5_cl_ui5__tree( ).
    DATA(result2) = NEW z5ui5_cl_ui5( lo_node ).
  ...
  METHOD _stringify.
    DATA(lo_node) = NEW z5ui5_cl_ui5( _node->mo_root ).
