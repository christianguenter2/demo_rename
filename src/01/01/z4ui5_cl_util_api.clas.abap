CLASS z4ui5_cl_util_api DEFINITION
  PUBLIC
  CREATE PUBLIC
  INHERITING FROM z4ui5_cl_stmpncfctn_api.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS url_param_create_url
      IMPORTING
        !t_params     TYPE z4ui5_if_types=>ty_t_name_value
      RETURNING
        VALUE(result) TYPE string.
    ...
    CLASS-METHODS url_param_get_tab
      IMPORTING
        !i_val           TYPE clike
      RETURNING
        VALUE(rt_params) TYPE z4ui5_if_types=>ty_t_name_value.
    ...
    CLASS-METHODS filter_get_token_range_mapping
      RETURNING
        VALUE(result) TYPE z4ui5_if_types=>ty_t_name_value.
...
CLASS z4ui5_cl_util_api IMPLEMENTATION.
  ...
  METHOD json_parse.
    TRY.
        ...
        z4ui5_cl_ajson=>parse( val )->to_abap(
          EXPORTING
          iv_corresponding = abap_true
          IMPORTING
            ev_container = data ).

      CATCH z4ui5_cx_ajson_error INTO DATA(x).
...
`x, nsis, objectivec, ocaml, pascal, perl, pgsql, php, plain_text, powershell, praat, prolog, properties, protobuf, python, r, razor, rdoc, rhtml, rst, ruby, rust, sass, scad, scala, scheme, scss, sh, sjs, smarty, snippets, soy_template, space, sql,` &&
  ...
  METHOD x_check_raise.
    ...
    IF when = abap_true.
      RAISE EXCEPTION TYPE z4ui5_cx_util_error EXPORTING val = v.
    ENDIF.
