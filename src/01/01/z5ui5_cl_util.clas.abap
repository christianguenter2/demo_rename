CLASS z5ui5_cl_util DEFINITION
  PUBLIC
  INHERITING FROM z5ui5_cl_util_api
  CREATE PUBLIC .
...
CLASS z5ui5_cl_util IMPLEMENTATION.
  ...
  METHOD db_load_by_handle.
    ...
    DATA lt_db TYPE STANDARD TABLE OF z5ui5_t_util_01 WITH EMPTY KEY.

    SELECT data
      FROM z5ui5_t_util_01
       WHERE
        uname = @uname
        AND handle = @handle
        AND handle2 = @handle2
  ...
  METHOD db_save.
    ...
    DATA(ls_db) = VALUE z5ui5_t_util_01(
        uname   = uname
        handle  = handle
        handle2 = handle2
        handle3 = handle3
    ...
    MODIFY z5ui5_t_util_01 FROM @ls_db.
    ASSERT sy-subrc = 0.
